 ...
 public:
  ...
  void finalize(void) {
    if (theDock.size() == 0)
      ...
      for (YachtData::iterator it = theDock.begin(); it != theDock.end();
           it++) {
        Yacht* y = it->second.get();
        // Yacht SeaBreeze remains on dock with passengers [Alice, John] and
        // supplies [Booze:85, Water:20].
        std::cout << "Yacht " << y->getName() << " remains on dock with "
                  << y->getInfo() << '.' << std::endl;
      }
  }
 ...
 protected:
  void processYacht(std::istream& istr) {
    ...
    if (event == "docking") {
      if (itFound != theDock.end()) {
        ...
        std::cout << "Error: Yacht " << name << " already on dock!"
                  << std::endl;
      } else {
  ...
  void processSupply(std::istream& istr) {
    ...
    if (action == "arriving") {
      ...
      if (itFound == theDock.end())
        std::cout << "Error: Yacht " << yachtName << " is not on dock!"
                  << std::endl;
      else {
...
int main() {
  ...
  std::string str;
  while (getline(std::cin, str) && str != "End") e.processCommand(str);
