...
class Persons {
 ...
 public:
  Person *findPerson(const std::string &firstName,
                     const std::string &secondName) {
    std::string id = generateId(firstName, secondName);

    auto it = persons.find(id);
    if (it != persons.end()) return it->second.get();
  ...
  static std::string generateId(const std::string &firstName,
                                const std::string &secondName) {
...
int main() {
  ...
  while (std::getline(std::cin, line) && line != ".") {
    ...
    Teacher *t =
        dynamic_cast<Teacher *>(data.findPerson(teacherName, teacherFamily));
    ...
    while (true) {
      ...
      Student *s =
          dynamic_cast<Student *>(data.findPerson(studentName, studentFamily));
