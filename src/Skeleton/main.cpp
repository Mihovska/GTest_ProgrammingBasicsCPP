...
void Engine::process(std::istream &is) {
  ...
  while (getline(is, line) && line != ".") {
    ...
    if (type[0] == 'M') {
      ...
      meetings.insert(std::pair<std::string, std::unique_ptr<Meeting>>(
          m->getId(), std::unique_ptr<Meeting>(m)));
    } else if (type[0] == 'C') {
...
void Engine::registerParticipant(const std::string &meetingID, Participant *c) {
  ...
  if (itM == meetings.end())
    // error message
    // `FirstName SecondName: No meeting MeetingID.`, for example `Stamat
    // Stamatov: No meeting StrangeMeeting.`.
    errorStream << c->toString() << ": No meeting " << meetingID << '.'
                << std::endl;
  else
...
void Engine::checkEmptyMeetings() const {
  for (auto itM = meetings.begin(); itM != meetings.end(); itM++) {
    ...
    if (m->participantCount() == 0) {
      errorStream << "Meeting " << m->getId() << " has no participants."
                  << std::endl;
    }
  }
}
...
void Engine::checkParticipantAvailability() const {
  // Create the processing order is all meetings: all meetings, ordered by
  // their:
  //   Start time of the meeting + the length of the meeting + the creation
  //   order of the meeting
  std::map<std::string, const Meeting *> processingOrder;
  for (auto itM = meetings.begin(); itM != meetings.end(); itM++) {
    Meeting *m = itM->second.get();
    std::ostringstream ostr;
    ostr << m->getDateTimePeriod().toString() << '_' << std::setw(4)
         << std::setfill('0') << m->getInputOrder();
    processingOrder.insert(
        std::pair<std::string, const Meeting *>(ostr.str(), m));
  }

  for (auto itM = processingOrder.begin(); itM != processingOrder.end();
       itM++) {
    const Meeting *m = itM->second;
    Meeting::Participants parts = m->getParticipants();

    // check each participants availability for the meetings *after* the meeting
    // in itM
      ...
      for (itNextM++; itNextM != processingOrder.end(); itNextM++) {
        ...
        if (meetingToCheck->hasParticipant(part)) {
          ...
          if (m->doesOverlapWith(meetingToCheck)) {
            // John Doe cannot attend InternalDiscussion meeting: still in
            // ClientMeeting meeting.
            errorStream << part->toString() << " cannot attend "
                        << meetingToCheck->getId() << " meeting: still in "
                        << m->getId() << " meeting." << std::endl;
          }
        }
      }
    }
