...
std::istream& operator>>(std::istream& is, TimeLength& tl) {
  ...
  ostringstream ostr;
  ostr << std::setw(2) << std::setfill('0') << lh << std::setw(2)
       << std::setfill('0') << lm;
...
std::istream& operator>>(std::istream& is, DateTime& dt) {
  ...
  ostringstream ostr;
  ostr << std::setw(4) << std::setfill('0') << year << std::setw(2)
       << std::setfill('0') << month << std::setw(2) << std::setfill('0') << day
       << std::setw(2) << std::setfill('0') << hour << std::setw(2)
       << std::setfill('0') << minute;
...
bool DateTimePeriod::doesOverlapWith(const DateTimePeriod& other) const {
  ...
  // check if *this finishes before other starts, AND other finishes before
  // *this ends,
  //   OR
  // other starts before *this, AND this finishes AFTER other
  if ((thisEndTime <= other) || (otherEndTime <= *this)) return false;
