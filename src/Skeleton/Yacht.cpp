...
bool Yacht::onSupplementArrival(Supplement *s) {
  ...
  if (sp != nullptr) {  // it's Supply
    ...
    ostr << "Supply " << s->getName() << " added to yacht " << getName()
         << ". New quantity: ";
    ...
    return sup == nullptr;  // if it was already found in the list - delete the
                            // `s` object
  } else {                  // it must be a Passenger!
    Supplement *sp = passengers.get(s->getName());
    if (sp != nullptr) {
      // Error: passenger John already exists in SeaBreeze crew!
      std::cout << "Error: passenger " << s->getName() << " already exists in "
                << getName() << " crew!" << std::endl;
      return false;  // another passenger with the same name exists - the new
                     // one must be deleted
    }
    passengers.insert(s);

    // Passenger Michael added to crew. New crew: [Emma, Michael].
    std::cout << "Passenger " << s->getName()
              << " added to crew. New crew: " << passengers.getInfo() << '.'
              << std::endl;
...
std::string Yacht::undock() const {
  ...
  // Yacht OceanQueen successfully undocked with passengers [empty] and supplies
  // [empty].
